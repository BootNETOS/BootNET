...
namespace BootNET.Network.SimpleHttpServer.RouteHandlers;
...
{
    public List<LiveFile> FilesPath = new();
    ...
    public FileSystemRouteHandler(string basePath)
    {
        ...
        foreach (var path in FilesPath) Console.WriteLine(path.Path + " registered.");
    ...
    private void DoTree(string directory)
    {
        ...
        foreach (var file in Directory.GetFiles(directory))
            FilesPath.Add(new LiveFile
            {
                Path = directory.Remove(0, BasePath.Length - 1) + file, Content = File.ReadAllBytes(directory + file)
            });

        for (var j = 0; j < directories.Length; j++) DoTree(directory + directories[j].ToLower() + "\\");
    }
