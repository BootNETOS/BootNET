...
public class FtpServer : IDisposable
{
    ...
    {
        if (Directory.Exists(directory) == false) throw new Exception("FTP server can't open specified directory.");
    ...
    public void Listen()
    {
        while (Listening)
        {
            ...
            Log("Client : New connection from " + client.RemoteEndPoint.Address);
            ;
    ...
    private void ReceiveNewClient(TcpClient client)
    {
        ...
        while (ftpClient.Control.IsConnected()) ReceiveRequest(ftpClient);
